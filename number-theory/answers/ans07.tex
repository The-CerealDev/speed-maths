...
\SpeedTitleBlock{Daily Number Theory Drill \#7 --- Answers \& Investigations}{David Akinyele-Aje}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{Prime Factorisation.}
        Knowing the prime factorisation of numbers is the key to counting divisors, finding GCDs, and determining properties of factorials. \seealso{A1, A6, B3, C1, D4}
  \item \textbf{Difference of Two Squares.}
        $(x-y)(x+y)$ appears everywhere. It simplifies evaluations like $a^2 - b^2$, factors polynomials, and solves Diophantine equations. \seealso{A5, B5, D3, D4, C7}
  \item \textbf{Completing the Rectangle/Square.}
        Equations like $xy + ax + by = c$ can always be factored by adding $ab$ to both sides, transforming them into $(x+b)(y+a) = c+ab$. \seealso{D1, C5, B5}
  \item \textbf{Modular Arithmetic Cycles.}
        Powers of numbers modulo $n$ always eventually fall into repeating cycles. You can compute the last digits of large powers by observing these short cycles. \seealso{A2, A7, B6, A9, B7}
  \item \textbf{Parity Arguments.}
        Often, testing even/odd cases or investigating divisibility by 2 and 4 can tightly restrict solutions to Diophantine equations, especially when primes are involved. \seealso{D5, D2, B4, A8, D1}
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{Forgetting 1 is not a prime.}
        When finding prime factors or solving equations with primes, remember that 2 is the smallest prime. 1 is just a unit. \seealso{A1, A6, C1, B9, A5}
  \item \textbf{Expanding unnecessarily.}
        Never expand an expression when you can keep it in factored form. The factors hold the secrets to divisibility and GCD. \seealso{A5, B1, D4, C7, B5}
  \item \textbf{Missing negative solutions in Diophantine equations.}
        When solving $x^2 - y^2 = d$, don't forget that negative factors multiply to positive results. Read carefully if the question restricts to positive integers. \seealso{B5, D1, D3, C7, C8}
  \item \textbf{Brute forcing modular equations.}
        For equations like $3x \equiv 2 \pmod{11}$, don't just guess numbers randomly; strategically add multiples of the modulus to the right-hand side until it is divisible by the coefficient. \seealso{A8, B7, C8, A4, B6}
  \item \textbf{Assuming a prime must be odd.}
        2 is the only even prime, and it is frequently the key that unlocks number theory puzzles involving squares and parity. \seealso{D5, C7, B5, A5, D2}
\end{enumerate}
