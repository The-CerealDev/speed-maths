...
def check_B9():
    ...
    for r in range(2, 41):
        assert [a for a in range(-500, 501) if r * a == a] == [-a for a in range(-500, 501) if r * -a == -a]
    candidate_count = counts.pop()
    options = {'A': candidate_count == 1, 'B': candidate_count == 2, 'C': candidate_count > 1000, 'D': False}
...
def check_C5():
    ...
    for r in range(2, 13):
        for c in range(1, 201):
            ...
            viable = [k for k, u in enumerate(magnitudes, start=1) if u.denominator == 1 and u != 0]
    ...
    options = {'A': ever_infinite, 'B': finite_everywhere, 'C': never_any, 'D': always_exactly_one}
    surviving = [letter for letter, holds in options.items() if holds]
    assert len(surviving) == 1, options
    return surviving[0]
...
def check_C8():
    ...
    def valid_next(S, n, used, window):
        return [a for a in range(1, window) if (S + a) % (n + 1) == 0 and a not in used]
...
def check_D3():
    ...
    for r in range(2, 13):
        for c in range(1, 301):
            ...
            viable = [k for k in range(1, 80) if fractions.Fraction(c, r ** k - 1).denominator == 1]
            assert all((k <= K for k in viable))
            assert len(viable) <= K
...
def check_D5():
    ...
    while remaining > 0:
        remaining -= 1
        steps += 1
    assert steps == pool_size
    r, c = (2, 45)
    viable = [k for k in range(1, 60) if fractions.Fraction(c, r ** k - 1).denominator == 1]
    assert viable and max(viable) < 60
    a = [1]
    for _ in range(200):
        a.append(a[-1] + 1)
    ...
    all_integer_sequences_bounded = any((all((unbounded(n) <= bound for n in range(1, bound + 2))) for bound in candidate_bounds))
    ...
    options = {'A': both_terminate, 'B': every_recurrence_periodic, 'C': all_integer_sequences_bounded, 'D': not both_terminate}
    surviving = [letter for letter, holds in options.items() if holds]
    assert len(surviving) == 1, options
    return surviving[0]
