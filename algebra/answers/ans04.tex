...
\SpeedTitleBlock{Daily Algebra Drill \#4 --- Answers \& Investigations}
...
\inv{With $ab=10$ and $a-b=3$: form $x^2-Sx+P=0$ where $S=a+b=\sqrt{(a-b)^2+4ab}=\sqrt{49}=7$. So $a,b$ are roots of $x^2-7x+10=0$, i.e.\ $(x-2)(x-5)=0$: $a=5,b=2$.}
...
\ans{$\dfrac{ab^2}{(a-b)(a+b)}=\dfrac{ab^2}{a^2-b^2}$}
\method{Common denominator $(a-b)(a+b)$: $\dfrac{ab(a+b)-a^2}{(a-b)(a+b)}=\dfrac{a(b^2+ab-a)}{(a^2-b^2)}$. Since numerator $= a^2b+ab^2-a^2 = ab^2+a^2(b-1)$ simplify: $ab(a+b)-a^2=a[b(a+b)-a]=a[ab+b^2-a]$. This leaves $\dfrac{a(ab+b^2-a)}{a^2-b^2}$.}
...
\inv{For what values of $x$ is this expression zero, undefined, or equal to $x$? Solving $(x-1)(x+1)^2=x^2$ gives a cubic --- use the rational root theorem to find integer solutions.}
...
\inv{Extend: show $n^2\equiv0,1,$ or $4\pmod8$ by checking all residues $n\equiv0,\ldots,7$. Hence show $n^2+n$ is always even (easier) and $n^2+n+2$ is never divisible by 4.}
