...
\SpeedTitleBlock{Daily Algebra Drill \#3 --- Answers \& Investigations}{David Akinyele-Aje}
...
Now find the minimum of $4x+\dfrac{9}{x}$ for $x>0$, and in general of
$ax+\dfrac{c}{x}$ for $a,c>0$. (\textit{Answer: $2\sqrt{ac}$, at $x=\sqrt{c/a}$.})}
...
$\alpha^2+\beta^2=(\alpha+\beta)^2-2\alpha\beta=9-3=6$.
...
\inv{Prove the identity $(n+1)!-n!-n\cdot(n-1)!=(n-1)!\cdot(n^2-n)$
...
Check in the original: LHS $=\tfrac{-7/5}{14/5}-\tfrac{9/5}{-6/5}=-\tfrac{1}{2}+\tfrac{3}{2}=1$. \checkmark}
...
\inv{Reprove the result via Cauchy--Schwarz:
$\left(\dfrac{1}{a}+\dfrac{1}{b}\right)(a+b)\geq(1+1)^2=4$, and $a+b=1$.
...
\ans{$x=1$ and $x=-1$\quad ($x=-1$ has multiplicity 3)}
\method{Group into two differences: $x^4+2x^3-2x-1=(x^4-1)+2x(x^2-1)$.
Both terms share the factor $x^2-1$:
$(x^2-1)(x^2+1)+2x(x^2-1)=(x^2-1)(x^2+2x+1)=(x-1)(x+1)(x+1)^2=(x-1)(x+1)^3$.
...
\textit{Part 2:} Apply Part 1 to the positive reals $\sqrt{a}$ and $\sqrt{b}$:
$\dfrac{\sqrt{a}}{\sqrt{b}}+\dfrac{\sqrt{b}}{\sqrt{a}}\geq2
\;\Rightarrow\; \dfrac{a+b}{\sqrt{ab}}\geq2
...
\ans{$2$\quad ($n=1$ and $n=5$)}
\method{$n^2-1=(n+3)(n-3)+8$, so $(n+3)\mid(n^2-1)$ iff $(n+3)\mid8$.
Since $n\geq1$, we need $n+3\geq4$, and the only divisors of 8 that are $\geq4$
are 4 and 8, giving $n=1$ and $n=5$. Exactly \textbf{2} values in range.}
...
\begin{enumerate}[leftmargin=2em, itemsep=5pt]
  \item \textbf{Remainder \& Factor Theorem.}
        ...
        To show $(x-a)$ is a factor: show $f(a)=0$. Instantaneous --- no long division. \seealso{A1, A2, B1}
  ...
  \item \textbf{AM--GM and the non-negativity of squares.}
        ...
        AM--GM ($\tfrac{a+b}{2}\geq\sqrt{ab}$) is the compressed form of this for products. \seealso{A5, C3, D3}

  \item \textbf{Completing the square in two (or more) variables.}
        $(x+h)^2+(y+k)^2$ is the canonical form for optimisation.
        The minimum is the squared distance from a point to the origin after shifting. \seealso{C2, C5}

  \item \textbf{Rational Root Theorem.}
        For $a_nx^n+\cdots+a_0$, rational roots have the form $p/q$ with $p\mid a_0$, $q\mid a_n$.
        Combined with the Factor Theorem: test candidates, divide out, repeat. \seealso{B1}

  \item \textbf{Divisibility via algebraic identity.}
        To show $(n+k)\mid f(n)$: write $f(n)=Q(n)\cdot(n+k)+R$ where $R$ is a constant.
        Then $(n+k)\mid f(n)$ iff $(n+k)\mid R$ --- reducing an infinite problem to a finite one. \seealso{B9, D5}
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em, itemsep=5pt]
  \item \textbf{Applying AM--GM to negative numbers.}
        ...
        Always check the sign condition before applying any mean inequality. \seealso{A5, D3}

  \item \textbf{Introducing extraneous solutions when squaring.}
        Squaring $\sqrt{A}=B$ requires $B\geq0$ \emph{first}.
        Always substitute back into the original equation to check. \seealso{C4}
  ...
  \item \textbf{Forgetting that the Rational Root Theorem gives candidates, not roots.}
        ...
        rules out impossible rational roots, it does not guarantee any exist. \seealso{B1}
  ...
  \item \textbf{Confusing divisibility direction.}
        ...
        $a\mid b$ means $b=ka$ for some integer $k$. \seealso{D5}
  ...
  \item \textbf{Treating multiplicity-$k$ roots as $k$ distinct solutions.}
        ...
        \emph{one} value of $x$. Count solutions, not multiplicities, unless the question asks. \seealso{D1}
\end{enumerate}
