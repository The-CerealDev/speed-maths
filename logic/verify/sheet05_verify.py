...
def check_A1():
    ...
    counterexample_exists = any((implies(P, Q) and Q and (not P) for P, Q in all_assignments(2)))
...
def check_A2():
    ...
    counterexample_exists = any((implies(P, Q) and (not P) and Q for P, Q in all_assignments(2)))
...
def check_A3():
    ...
    flawed_premise = lambda n: n % 2 == 0
    assert not all((flawed_premise(n) for n in range(1, 200))), 'the flawed premise must not be universally true, or the argument would not be flawed'
...
def check_A6():
    ...
    error_exists = not conclusion_true
...
def check_B3():
    ...
    assert math.isclose(math.sqrt(9 + 7), 9 - 5)
    lhs2, rhs2 = (math.sqrt(2 + 7), 2 - 5)
    assert rhs2 < 0 <= lhs2
    assert not math.isclose(lhs2, rhs2)
    return expected_ans
...
def check_B4():
    ...
    for n in range(-200, 201):
        ...
        two_AB_erroneous = Fraction(4 * n + 1 - n - 3)
        assert two_AB_erroneous == Fraction(3 * n - 2)
        assert two_AB_correct != two_AB_erroneous
...
def check_B8():
    ...
    for n in range(-300, 301):
        if n % 3 == 0:
            assert n ** 2 % 3 == 0
    mismatch_with_original = any((implies(A, B) != implies(not A, not B) for A, B in all_assignments(2)))
    mismatch_with_contrapositive = any((implies(A, B) != implies(B, A) for A, B in all_assignments(2)))
...
def check_B10():
    ...
    mismatch_exists = any((all((not p for p in pattern)) != (not all(pattern)) for pattern in itertools.product([False, True], repeat=5)))
...
def check_C3():
    ...
    for k in range(-150, 151):
        n_even = 2 * k
        assert (n_even ** 2 + n_even) % 2 == 0
        n_odd = 2 * k + 1
        assert (n_odd ** 2 + n_odd) % 2 == 0
    return expected_ans
...
def check_C6():
    ...
    assert math.isclose(AB, DA)
    assert math.isclose(BC, CD)
    assert not math.isclose(AB, BC)
    AC = (C[0] - A[0], C[1] - A[1])
    BD = (D[0] - B[0], D[1] - B[1])
...
def check_D4():
    ...
    for p in range(5, LIMIT):
        if is_prime(p):
            assert p % 6 in (1, 5)
    assert 25 % 6 == 1
    assert not is_prime(25) and 25 == 5 ** 2
    assert 6 * 0 + 3 == 3
    assert is_prime(3)
    return expected_ans
