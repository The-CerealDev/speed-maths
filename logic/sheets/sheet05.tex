...
\SpeedMeta{Flawed proof step diagnosis and structural traps}{flawed proof, error diagnosis, converse error, extraneous roots}
\noindent\textit{New toolkit today: Flawed Proof Diagnosis $\cdot$ Sign Distribution Traps $\cdot$ Extraneous Solutions $\cdot$ Domain Violations.}
...
\noindent\textit{Rapid error recognition: algebraic, functional, and logical traps.}
...
\item[B)] The student has improperly swapped the order of quantifiers.
...
Assess this answer.
\begin{itemize}
\item[A)] The claim $(*)$ is true, so the disproof must contain a mistake.
\item[B)] A valid counterexample requires $x$ and $y$ to have opposite signs.
\item[C)] A single pair of values is never enough evidence against a ``for every'' claim.
\item[D)] The student's reasoning is fully sound.
\end{itemize}
...
\item[B)] It is incorrect; the student proved the converse instead.
\item[C)] It is incorrect because line II is false.
\item[D)] It is incorrect because line III is false.
\item[E)] There is no flaw; assuming $u,v$ exist first is a valid proof technique.
\end{itemize}
...
\item[B)] The argument proves necessity but fails to prove sufficiency.
