...
\ans{1}
\method{For $n=1$, $u_1 = 1$ and $2^1 - 1 = 1$. The base case is verified at $n=1$.}
\inv{Compute $u_2$ and $u_3$ to verify the inductive step holds for the first two transitions.}

%── A2 ──────────────────────────────────────────────────────────────────────────
\item To prove $P(n)$ for all integers $n\geq4$ by induction, what is the base-case integer?

\ans{4}
\method{The base case is always the smallest value in the claimed range, which is $n=4$.}
\inv{If the claim were instead ``for all $n\geq4$ except $n=5$'', how would induction have to be structured?}

%── A3 ──────────────────────────────────────────────────────────────────────────
\item If $P(k)\implies P(k+1)$ is proved for all $k\geq1$, but no base case is ever established, has $P(n)$ been proved for any $n$?

\ans{False.}
\method{Without a base case, an inductive step is purely conditional ($P(k) \implies P(k+1)$). If $P(n)$ is false for every $n$, the implication is vacuously true, yet nothing has been proved.}
...
\ans{$n^2$}
\method{Telescoping the difference $u_{n+1} - u_n = 2n+1$ gives $u_n = u_1 + \sum_{k=1}^{n-1} (2k+1) = 1 + (n^2-1) = n^2$.}
\inv{Verify by induction that $u_n = n^2$ satisfies $u_{n+1} = (n+1)^2$.}
...
\ans{5}
\method{Check: $2^5=32>25=5^2$, true. (Note $2^4=16=4^2$ and $2^3=8<9=3^2$, so the claim genuinely fails at $n=3, 4$, requiring $n=5$ as the base case.)}
...
\ans{$\exists k\geq1 \text{ such that } P(k) \land \neg P(k+1)$}
...
\ans{2}
\method{Because the recurrence step for $u_{n+2}$ requires the values of the two preceding terms ($u_{n+1}$ and $u_n$), exactly two independent base cases ($u_1$ and $u_2$) are required to determine all subsequent terms.}
\inv{Solve the characteristic equation $r^2 - 3r + 2 = 0$ to find the closed-form expression for $u_n$.}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{The Inductive Step as an Implication Chain.}
        The inductive step proves $P(k)\Rightarrow P(k+1)$ universally; without an established base case $P(n_0)$, the implication chain remains an unanchored abstraction that proves nothing. \seealso{A1, A2, A3, C8, B10}
  \item \textbf{Shifted and Multi-Step Base Cases.}
        Statements valid only for $n\ge n_0$ (e.g.\ $2^n>n^2$ for $n\ge5$) must anchor at $n_0$; recurrences or two-step steps $P(k)\Rightarrow P(k+2)$ require multiple base cases to avoid skipping parity classes. \seealso{A2, A6, A8, B10, C3}
  \item \textbf{Strong Induction for Decomposable Structures.}
        When an inductive step decomposes an integer or recurrence into smaller pieces of unknown size rather than just $k$ and $1$ (e.g.\ prime factorisation or $t_{n+1}=t\cdot t_n-t_{n-1}$), strong induction assuming all $P(1),\dots,P(k)$ is mandatory. \seealso{A10, C4, C6, D1, D2}
  \item \textbf{Inequality Induction and Sign Preservation.}
        Multiplying an inductive inequality like $(1+x)^k\ge 1+kx$ by $(1+x)$ to reach $k+1$ preserves direction only when the multiplier is strictly positive ($x>-1$); negative multipliers flip inequalities. \seealso{B6, B7, D3, D5}
  \item \textbf{Strengthening the Inductive Hypothesis.}
        Proving a tighter bound like $\sum_{k=1}^n \frac{1}{k^2}<2-\frac{1}{n}$ is often easier by induction than proving the looser $\sum \frac{1}{k^2}<2$, because the stronger claim provides an extra negative decrement to absorb new terms. \seealso{D5, B8, D4}
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{The Step-One Overlap Breakdown.}
        Assuming the transition from $k$ to $k+1$ works identically for all $k$ overlooks the breakdown at $k=1\to2$ (the ``all horses same colour'' fallacy), where the intersection between two sub-collections is empty. \seealso{C1, A7, C8}
  \item \textbf{Skipping Parity in Step-Size Two Induction.}
        Proving $P(k)\Rightarrow P(k+2)$ with only $P(1)$ proved establishes $P(n)$ exclusively for odd integers; an independent base case $P(2)$ is required for even integers. \seealso{A8, B10, D1}
  \item \textbf{Assuming Failed $P(1)$ Invalidates the Entire Proposition.}
        If $P(1)$ or $P(2)$ fails, the statement may still hold for all $n\ge n_0$ (e.g.\ $n!\ge 3^n$ for $n\ge7$); the valid base case simply begins at a higher threshold. \seealso{A6, A9, C3, C5}
  \item \textbf{Using Weak Induction Where Subproblems Vary in Size.}
        Attempting to prove claims about integer factorisation using weak induction fails because dividing $k+1$ by a prime produces an integer strictly smaller than $k$, not necessarily equal to $k$. \seealso{A10, C4, C6, D2}
  \item \textbf{Applying Induction Across Continuous Domains.}
        Mathematical induction relies on the well-ordering of natural numbers; it cannot be applied directly across continuous intervals or real numbers without discretization. \seealso{A5, D3}
\end{enumerate}
