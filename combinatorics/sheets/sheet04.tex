...
\SpeedTitleBlock{Daily Combinatorics Drill \#4}{David Akinyele-Aje}
\SpeedMeta{Binomial identities and double counting}{binomial coefficients, greatest coefficient, double counting}
...
\item Express $\dbinom{9}{4}+\dbinom{9}{5}$ as a single binomial coefficient.
...
\item Find the coefficient of $x^5$ in $(1+2x)^4(1+x)^3$.
...
\item In the expansion of $(1+3x)^n$, the coefficient of $x^5$ is exactly three times the coefficient of $x^4$. Find $n$.
...
\item By substituting a suitable value into an expansion, evaluate $\;\displaystyle\sum_{k=0}^{6}\binom{6}{k}2^k$.
...
\item Prove that exactly four of the coefficients $\dbinom{12}{k}$, $0\leq k\leq12$, are odd. \textit{\small(cf.\ TMUA 2023 P1 Q6)}
