...
%── C1 ──────────────────────────────────────────────────────────────────────────
\item Beads of 3 colours: guarantee 5 alike?

\ans{$3\cdot4+1=13$}
\method{Worst case 4 of each colour; the 13th bead is a fifth of something.}
\inv{Strong pigeonhole in one line. Now invert the question: with 30 beads of 3 colours, what group size is \emph{guaranteed} to exist? ($\lceil30/3\rceil=10$ of some colour.) Forcing-number vs guaranteed-size --- keep the two directions straight.}
...
%── C5 ──────────────────────────────────────────────────────────────────────────
\item Group size guaranteeing 4 share a birth month?

\ans{$12\cdot3+1=37$}
\method{Worst case: 3 in every month (36 people). One more forces a fourth.}
\inv{Real-world calibration: school year groups of 37+ always contain a birth-month quartet --- check your own class. Strong pigeonhole with $k=12$, $m=4$; write the general $k(m-1)+1$ and test it against every A/C question so far.}
...
%── D3 ──────────────────────────────────────────────────────────────────────────
\item Two guests with equal handshake counts.

\ans{Proof below.}
\method{\emph{Proof.} Each of the $n$ guests has a handshake count in $\{0,1,\ldots,n-1\}$ --- $n$ possible values. Suppose all counts were distinct; then every value occurs exactly once, so some guest $A$ has count $n-1$ and some guest $B$ has count $0$. But $A$ shook hands with everyone else, including $B$, contradicting $B$'s count of $0$. Hence not all counts are distinct: two guests share a count. $\square$}
\inv{Note the structure: pigeonhole fails at first ($n$ values, $n$ people), so we \emph{assume distinctness and hunt a contradiction} between the forced extremes. Extend: must THREE guests share a count? (No --- construct a party with counts as spread as possible.)}

%── D4 ──────────────────────────────────────────────────────────────────────────
\item 20 students, 2 problems each, 5 problems solved equally: determine $k$.

\ans{$k=8$}
\method{\emph{Proof.} Count (student, solved problem) pairs two ways. By students: each of 20 students contributes exactly 2 pairs: $40$. By problems: each of the 5 problems is solved by $k$ students, contributing $5k$ pairs. The same set counted twice: $5k=40$, so $k=8$. Feasibility: any assignment of students to the $\binom52=10$ problem-pairs with each problem in $8$ chosen pairs works --- e.g.\ give each of the 10 problem-pairs exactly 2 students. $\square$}
\inv{The determination is two lines; the \emph{feasibility construction} is what BMO1 markers look for and students forget. Harder: if instead every PAIR of problems is solved by exactly one common student, how many students? (10 --- one per pair; the double count now runs over problem-pairs.)}
