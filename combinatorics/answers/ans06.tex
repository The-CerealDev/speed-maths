...
\SpeedTitleBlock{Daily Combinatorics Drill \#6 --- Answers \& Investigations}{David Akinyele-Aje}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{Basic pigeonhole.} $k+1$ objects in $k$ boxes: some box holds two. The craft is in \emph{designing} the boxes: residues, sum-pairs, quarter-squares, parity classes. \seealso{A3, A5, A8, B4, D1, D2}
  \item \textbf{Strong pigeonhole.} Forcing $m$ alike among $k$ types needs $k(m-1)+1$ objects. Equivalently: some box holds at least the ceiling of the average. \seealso{A9, B9, C1}
  \item \textbf{Worst case first.} Every guarantee number is (largest configuration that dodges) $+1$. A complete answer exhibits the dodge AND proves the force. \seealso{A2, B1, B4, C1, C2}
  \item \textbf{Double counting.} Count (X, Y) incidences by X and by Y: $a|X|=b|Y|$. Grids, committees, degree sums --- and an integrality feasibility test for free. \seealso{A6, A7, B5, B10, C6, D3}
  \item \textbf{Shrink the boxes by contradiction.} When boxes equal pigeons, show two boxes cannot both be occupied (counts 0 and $n-1$) --- then pigeonhole closes the argument. \seealso{B8}
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{Answering the force without the dodge.} ``7 diners force adjacency'' is half marks until you seat 6 who don't. Guarantee $=$ construction $+$ pigeonhole. \seealso{B1, B4, C2, D1}
  \item \textbf{Off-by-one in strong pigeonhole.} It is $k(m-1)+1$, not $km$ or $km+1$. Re-derive from the worst case each time rather than recalling. \seealso{A9, B9, C1}
  \item \textbf{Boxes that aren't disjoint or don't cover.} Sum-pair and difference boxes must partition the set --- check both properties explicitly before counting pigeons. \seealso{B4, C2, C8, D1, D2}
  \item \textbf{Double counting with one side wrong.} ``Each of 20 clubs of 7, everyone in 3'' has no integer solution --- test divisibility before trusting a puzzle's premises. \seealso{B10, C6}
  \item \textbf{Vague write-ups.} ``By pigeonhole, done'' scores nothing. Name the pigeons, name the boxes, state which two collide and what that collision gives you. \seealso{B8, D1, D3, D5}
\end{enumerate}
