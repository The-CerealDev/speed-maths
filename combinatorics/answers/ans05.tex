...
%── A5 ──────────────────────────────────────────────────────────────────────────
\item Multiples of 3 or 5 up to 100?

\ans{$33+20-6=47$}
\method{Multiples of 15 were counted twice; subtract them once.}
\inv{The overlap class is ``divisible by $\mathrm{lcm}$'', not ``by the product'' --- for 4 and 6 the overlap is multiples of 12, not 24. Recompute this question with 4 and 6 to catch the distinction.}
...
%── C1 ──────────────────────────────────────────────────────────────────────────
\item Positive quadruples, $x+y+z+w=15$?

\ans{$\dbinom{14}{3}=364$}
\method{Bars-in-gaps: 15 stars, choose 3 of the 14 gaps.}
\inv{Also the number of three-element subsets of $\{1,\ldots,14\}$ --- find the direct bijection (the bar positions ARE the subset). Stars-and-bars is secretly always a subset count.}
...
%── C8 ──────────────────────────────────────────────────────────────────────────
\item Positive integers below 1000 with digit sum 9?

\ans{$\dbinom{11}{2}=55$}
\method{Write each number as a 3-digit string $d_1d_2d_3$ (leading zeros allowed, from 001 to 999): need $d_1+d_2+d_3=9$ with digits $0$--$9$. The cap never bites (no digit can exceed 9 when the sum is 9), so pure stars and bars: $\binom{11}{2}=55$. The string 000 has sum 0, so nothing was wrongly included.}
\inv{Digit-sum 10 needs the cap: $\binom{12}{2}-3=63$ (subtract the three strings with a digit 10... check via the violation shift). Where exactly does the cap start biting, and why at sum 10? This ``pad with leading zeros'' recoding is a bijection worth naming in written solutions.}
...
%── D5 ──────────────────────────────────────────────────────────────────────────
\item 1--$10^6$: neither squares nor cubes?
