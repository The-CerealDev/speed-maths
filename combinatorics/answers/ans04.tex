...
\SpeedTitleBlock{Daily Combinatorics Drill \#4 --- Answers \& Investigations}{David Akinyele-Aje}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{The general term.} Write $\binom{n}{k}a^{n-k}b^k$ \emph{with the constants inside}, solve the degree equation for $k$, then compute. Never expand what you can index. \seealso{A1, B1, B3, B5}
  \item \textbf{Substitute values into identities.} $x=1$: coefficient sum. $x=-1$: alternating sum. Add/subtract the two: even/odd extraction. Every substitution is a theorem. \seealso{A5, A8, C1, C7}
  \item \textbf{Pascal's rule = in-or-out.} $\binom{n}{r}=\binom{n-1}{r-1}+\binom{n-1}{r}$ is the committee split from sheet 2, and it generates the whole triangle. \seealso{A3, A7, C8, D1, D4}
  \item \textbf{Count one thing two ways.} Vandermonde, hockey stick, committee--chair: pick the right scenario and the identity proves itself in a sentence. \seealso{C6, D2, D4}
  \item \textbf{Binomials mod small primes.} $(1+x)^2\equiv1+x^2\pmod2$ propagates to $(1+x)^{2^m}\equiv1+x^{2^m}$: parity patterns of whole rows fall out (Lucas/Sierpi\'nski). \seealso{D3, D5}
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{Dropping the constant's power.} In $(2+3x)^5$ the $2^{5-k}$ is compulsory. Coefficient-sum check at $x=1$ catches this in seconds. \seealso{B1, B6, C5}
  \item \textbf{Sign slips with $(a-b)^n$.} Keep $(-b)^k$ intact inside the general term; decide the sign from the parity of $k$, never by feel. \seealso{A9, B6, C5}
  \item \textbf{Expanding both factors fully for one coefficient.} Harvest each factor only to the degree needed and zip. Full expansion is slow and error-prone. \seealso{B2, B6, C5}
  \item \textbf{Assuming symmetric-looking sums need algebra.} $\sum\binom{10}{k}^2$ falls to a story about choosing 10 people. Reach for the committee before the formula sheet. \seealso{C6, D2}
  \item \textbf{Using primality nowhere.} If your proof of D5 never invokes that $p$ is prime, it proves the false statement $4\mid\binom{4}{2}$. Locate where every hypothesis earns its keep. \seealso{D5}
\end{enumerate}
