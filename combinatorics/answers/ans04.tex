...
%── A7 ──────────────────────────────────────────────────────────────────────────
\item $\dbinom{9}{4}+\dbinom{9}{5}$ as one coefficient.

\ans{$\dbinom{10}{5}=252$}
\method{Pascal's rule: adjacent entries sum to the entry below.}
...
%── B2 ──────────────────────────────────────────────────────────────────────────
\item Coefficient of $x^5$ in $(1+x)^4(1+x)^3$.

\ans{$\dbinom{7}{5}=21$}
\method{Multiply the bases first: $(1+x)^7$. Never expand the factors separately when they share a base.}
\inv{Expanding separately and convolving gives $\sum_k\binom{4}{k}\binom{3}{5-k}=\binom{7}{5}$ --- you have accidentally proved a Vandermonde identity. D2 weaponises this.}
...
\ans{$2^9=512$}
\method{Add the $x=1$ and $x=-1$ substitutions of $(1+x)^{10}$: odd terms cancel, even terms double. $(2^{10}+0)/2$.}
...
\inv{The degree lattice here is $12,9,6,3,0,-3,-6$. Which is the constant term? ($k=4$: $\binom{6}{4}=15$ --- equal to the $x^6$ coefficient, by which symmetry?)}
...
%── C4 ──────────────────────────────────────────────────────────────────────────
\item Coefficients of $x^4$ and $x^5$ in $(1+x)^n$ equal. Find $n$.

\ans{$n=9$}
\method{$\binom{n}{4}=\binom{n}{5}\Rightarrow4+5=n$.}
\inv{Equal \emph{adjacent} coefficients happen only for odd $n$ at the centre --- connect to A10's ``two equal maxima''. For $(1+2x)^n$, adjacent equality instead solves $\frac{2(n-k)}{k+1}=1$: find an $(n,k)$ example.}
...
%── C7 ──────────────────────────────────────────────────────────────────────────
\item Constant term of $\left(x-\frac{2}{x}\right)^8$.

\ans{$\dbinom{8}{4}(-2)^4=70\cdot16=1\,120$}
\method{Degree $8-2k=0$ gives $k=4$; the $(-2)^4$ is positive. Sign discipline: write the $(-2)^k$ \emph{inside} the general term from the start.}
\inv{What is the coefficient of $x^2$ in the same expansion, and is it positive or negative? ($k=3$: $\binom{8}{3}(-2)^3=-448$.) Predict the sign from the parity of $k$ before computing.}
...
%── D3 ──────────────────────────────────────────────────────────────────────────
\item For how many $k$ is $\dbinom{12}{k}$ odd? \textit{\small(cf.\ TMUA 2023 P1 Q6)}

\ans{$4$\quad ($k=0,4,8,12$)}
\method{Work mod 2 with the factorisation $(1+x)^{12}=\left((1+x)^4\right)^3$. Mod 2, $(1+x)^2\equiv1+x^2$ and hence $(1+x)^4\equiv1+x^4$, so $(1+x)^{12}\equiv(1+x^4)^3=1+3x^4+3x^8+x^{12}\equiv1+x^4+x^8+x^{12}\pmod2$. The odd coefficients sit exactly at $k=0,4,8,12$: four of them.}
