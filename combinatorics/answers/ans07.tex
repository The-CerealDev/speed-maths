...
\SpeedTitleBlock{Daily Combinatorics Drill \#7 --- Answers \& Investigations}
...
%── B8 ──────────────────────────────────────────────────────────────────────────
\item Snaps to break a $4\times6$ bar into 24 squares?
...
%── B10 ─────────────────────────────────────────────────────────────────────────
\item Prove $F_1+\cdots+F_n=F_{n+2}-1$; evaluate for $n=8$.

\ans{$54$}
\method{\emph{Proof (telescoping).} Since $F_k=F_{k+2}-F_{k+1}$, summing $k=1$ to $n$ collapses: $\sum_{k=1}^n F_k=\sum(F_{k+2}-F_{k+1})=F_{n+2}-F_2=F_{n+2}-1$. For $n=8$: $F_{10}-1=55-1=54$. $\square$}
\inv{Telescoping a recurrence is the fastest route to its running sums. Derive $\sum F_i^2=F_nF_{n+1}$ the same way (from $F_i^2=F_i F_{i+1}-F_{i-1}F_i$) --- a gorgeous identity with a one-line proof.}
...
%── C6 ──────────────────────────────────────────────────────────────────────────
\item Derive the Hanoi recurrence; give the 10-disk count.

\ans{$1023$}
\method{To move $n$ disks, you must move the top $n-1$ to the spare peg ($H(n-1)$ moves), move the largest ($1$ move), then move the $n-1$ back ($H(n-1)$): $H(n)=2H(n-1)+1$. Unfolding: $H(n)=2^n-1$, so $H(10)=1023$.}
\inv{Unfold the recurrence symbolically: $H(n)=2H(n-1)+1=2^2H(n-2)+2+1=\cdots=2^n-1$. The ``$+1$ per level doubled'' becomes the geometric sum $2^{n}-1$. This telescoping-by-substitution solves every $a_n=ra_{n-1}+c$.}
...
%── D5 ──────────────────────────────────────────────────────────────────────────
\item Take 1 or 2 stones from 2025; last stone wins. Who wins?

\ans{The second player.}
\method{\emph{Proof.} Call the number of stones a player must move from their \emph{position}, and call a position \emph{losing} if the player to move loses under optimal play. We claim the losing positions are exactly the multiples of 3. From a multiple of 3, every move (take 1 or 2) leaves a non-multiple of 3 for the opponent; from a non-multiple of 3, the mover can always take 1 or 2 to leave a multiple of 3. Since position $0$ is losing (the previous player took the last stone and won), it follows by induction that $0,3,6,\ldots$ are losing and all other positions are winning. The first player starts at $2025=3\times675$, a multiple of 3 --- a losing position. Hence the \emph{second} player wins, by always returning the pile to a multiple of 3. $\square$}
\inv{The winning strategy is an invariant: the winner keeps the pile $\equiv0\pmod3$ after each of their moves, which they always can because $1+2=3$ spans a full residue step. Generalise to ``take 1, 2, or $k$'': the losing positions become the multiples of $k+1$. What changes in \emph{mis\`ere} play, where taking the last stone loses? Re-derive the losing set.}
