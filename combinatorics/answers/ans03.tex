...
%── A7 ──────────────────────────────────────────────────────────────────────────
\item Arrangements of SPEED with the two E's together?

\ans{$4!=24$}
\method{Glue EE into one block: the objects S, P, D, [EE] are 4 distinct things. (The E's are identical, so no $\times2$ for internal order.)}
\inv{Contrast with gluing two \emph{different} letters, which costs a factor 2. Then check the complement against sheet 1: total arrangements of SPEED $=60$, so E's apart $=60-24=36$. Verify by the gap method.}
...
%── C1 ──────────────────────────────────────────────────────────────────────────
\item Eight in a row, three friends consecutive?

\ans{$3!\cdot6!=4\,320$}
\method{Glue the three into a block (internal orders $3!$), then 6 objects in a row.}
\inv{Generalise: $k$ specified people consecutive among $n$ gives $k!\,(n-k+1)!$. Sanity-check the formula against B1 ($k=2$, $n=6$).}

%── C2 ──────────────────────────────────────────────────────────────────────────
\item Same, but the three friends in height order?

\ans{$6!=720$}
\method{The block's internal order is now forced: drop the $3!$.}
...
%── C3 ──────────────────────────────────────────────────────────────────────────
\item Arrangements of DIVIDED?

\ans{$\dfrac{7!}{3!\,2!}=420$}
\method{Seven letters: D$\times$3, I$\times$2, V, E. $\frac{5040}{12}=420$.}
\inv{How many arrangements \emph{start and end with D}? (Fix two D's at the ends; arrange the middle five letters D, I, I, V, E: $\frac{5!}{2!}=60$.) Note you don't choose \emph{which} D's --- they are identical.}
...
%── C6 ──────────────────────────────────────────────────────────────────────────
\item Five-digit even numbers using $1,2,3,4,5$ once each?

\ans{$2\cdot4!=48$}
\method{Units digit must be 2 or 4 (two choices); the rest arrange freely.}
\inv{How many are divisible by 4? A number is divisible by 4 exactly when its last two digits are. The valid distinct-digit endings from $\{1,\ldots,5\}$ are 12, 24, 32, 52 --- four endings, each completed by $3!$ arrangements of the remaining digits: $4\cdot6=24$.}
...
%── D1 ──────────────────────────────────────────────────────────────────────────
\item Seven-digit numbers from $1$--$7$, odd digits in increasing order?

\ans{$\dfrac{7!}{4!}=210$}
\method{The four odd digits $1,3,5,7$ appear in some order in every arrangement; exactly one of the $4!$ orders is increasing. So divide: $\frac{5040}{24}=210$. Equivalently: choose which 4 positions hold the odds ($\binom{7}{4}=35$, order then forced) and arrange the three evens ($3!$): $35\cdot6=210$.}
\inv{Two proofs, one answer --- the divide-by-order trick and the choose-positions trick are secretly the same computation: $\binom{7}{4}\cdot3!=\frac{7!}{4!}$. Now count arrangements where the odds increase AND the evens decrease ($\binom{7}{4}\cdot1=35$).}
...
%── D5 ──────────────────────────────────────────────────────────────────────────
\item Eight knights round a table, three feuding knights pairwise non-adjacent?

\ans{$4!\cdot5\cdot4\cdot3=1\,440$}
\method{Gap method, circular edition. Seat the five peaceful knights first: $(5-1)!=24$ circular orders, creating exactly 5 gaps between them. Place the three feuding knights into three \emph{different} gaps (one per gap --- two in one gap would be adjacent to each other): $5\cdot4\cdot3=60$. Total $24\cdot60=1\,440$.}
\inv{Why must each chosen gap hold exactly one feuder? And why did the circle change the gap count (5 gaps from 5 people, versus 6 gaps from 5 people in a row)? Generalise: $n$ knights, $k$ feuding, count $=(n-k-1)!\cdot\frac{(n-k)!}{(n-2k)!}$ --- test it against B4's row analogue and this question.}
