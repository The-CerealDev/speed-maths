...
\SpeedTitleBlock{Daily Combinatorics Drill \#2 --- Answers \& Investigations}{David Akinyele-Aje}
...
%── D5 ──────────────────────────────────────────────────────────────────────────
\item A committee of 4 is chosen from 9 people (5 women including Priya, 4 men). Consider statements: (A) Priya is on the committee, (B) exactly 2 women are on the committee, (C) at least 3 men are on the committee. Exactly one of the three statements is true. How many committees satisfy this? \textit{\small(after TMUA Practice Paper 2 Q4)}

\ans{$81$}
\method{Brute force over $\binom94=126$ committees. Let $w$ be number of women. (A) $w\ge1$ and Priya in, (B) $w=2$, (C) $w\le1$. Exactly one true splits into 3 disjoint cases: (A only) $w\ge2$ with Priya and $w\neq2$ and $w>1$ → count $=...$, (B only) $w=2$ without Priya and $w>1$ → ..., (C only) $w\le1$ without Priya and $w\neq2$ → .... Summing gives $81$. Equivalently, enumerate all 126 and test $[\![A]\!]+[\![B]\!]+[\![C]\!]=1$; 81 pass.}
\inv{The cited TMUA question has 5 urns each with a statement exactly one true — same \emph{exactly-one-true} logic as here, but here the counting is committees not balls. This hybrid is how TMUA wraps a counting question in a logic stem. Try variant: what if \emph{exactly two} statements were true?}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{Choose vs arrange.} $\binom{n}{r}=\frac{{}^nP_r}{r!}$: divide by $r!$ exactly when the selection is unordered. Ask ``does swapping two chosen objects give a new outcome?'' \seealso{A4, A8, A10, B2}
  \item \textbf{Symmetry flip.} $\binom{n}{r}=\binom{n}{n-r}$: always compute the smaller index. $\binom{100}{98}$ is a two-second question. \seealso{A3, A9}
  \item \textbf{Complementary counting.} ``At least one X'' $=$ total $-$ ``no X''. The complement of a hard condition is usually one clean product. \seealso{A6, C2, C4, C5, C8}
  \item \textbf{Committee--chair double counting.} $r\binom{n}{r}=n\binom{n-1}{r-1}$: proving identities by counting a scenario two ways beats algebra. \seealso{B5, B8, C5}
  \item \textbf{Objects $\leftrightarrow$ subsets.} Binary strings, lattice paths, handshakes, grid rectangles, chord crossings --- all are ``choose a subset'' in costume. Find the subset. \seealso{B3, B4, B6, B10, D1}
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  \item \textbf{Ordering an unordered selection.} A committee counted with ${}^nP_r$ is overcounted by a factor of $r!$. Decide ordered/unordered \emph{before} touching numbers. \seealso{A4, A8, A10, B2}
  \item \textbf{The ``guarantee the quota'' overcount.} ``Choose 4 women, then anyone else'' double-counts committees with 5 women (C1). Quota problems need exact cases or a complement --- never ``reserve then top up''. \seealso{C1}
  \item \textbf{Adding overlapping cases.} Cases must be \emph{disjoint} before you add. ``Contains 1'' and ``contains 2'' overlap; either subtract the overlap or complement. \seealso{C8, D5}
  \item \textbf{Forgetting forced choices are free.} All four aces in the hand is a factor of 1, not $\binom{4}{4}$ worth of anxiety --- but \emph{exactly} three aces needs $\binom{4}{3}$. \seealso{C6}
  \item \textbf{Dividing by symmetry that isn't there.} Handshake circles divide by rotations \emph{and} reflections ($\frac{5!}{2}$); labelled pair-splits divide by $2$ only when the two groups are indistinguishable. Name the symmetry before dividing by it. \seealso{B3, D4}
\end{enumerate}
