...
%── B7 ──────────────────────────────────────────────────────────────────────────
\item How many 3-letter strings (repeats allowed) contain at least one vowel?

\ans{$26^3-21^3=17\,576-9\,261=8\,315$}
\method{Complement: strings with \emph{no} vowel use 21 consonants: $21^3$. Subtract from the total.}
\inv{``At least one'' is the complement battle-cry. Try ``at least two vowels'' --- now the complement (zero or one vowel) needs two cases: $21^3+3\cdot5\cdot21^2$. Check the three counts (0, 1, $\geq2$ vowels) sum to $26^3$.}
...
%── C4 ──────────────────────────────────────────────────────────────────────────
\item Triangles from 8 points on a circle; how many use point $A$?

\ans{$\dbinom{8}{3}=56$ in total;\quad $\dbinom{7}{2}=21$ through $A$}
\method{All triples work (no three points on a circle are collinear). Through $A$: fix $A$, choose 2 partners.}
\inv{Check by symmetry: each of the 56 triangles has 3 vertices, so vertex-uses total $56\cdot3=168=8\cdot21$ --- each point is used 21 times. Counting incidences two ways strikes again.}

%── C5 ──────────────────────────────────────────────────────────────────────────
\item Teams of 6 from 6 boys and 6 girls with equal numbers of each.

\ans{$\dbinom{6}{3}^2=20^2=400$}
\method{Equal numbers in a team of 6 means 3 boys and 3 girls: $\binom{6}{3}\binom{6}{3}$.}
\inv{Sum over all splits: $\sum_{k}\binom{6}{k}\binom{6}{6-k}=\binom{12}{6}=924$ --- Vandermonde's identity, proved by asking how many boys ended up in an unrestricted team. Verify the $k=3$ term is your 400.}

%── C6 ──────────────────────────────────────────────────────────────────────────
\item 5-card hands from 52 cards containing all four aces.

\ans{$\dbinom{48}{1}=48$}
\method{The four aces are forced; only the fifth card is a decision.}
\inv{Harder: hands with \emph{exactly three} aces: $\binom{4}{3}\binom{48}{2}=4512$. Why must you choose \emph{which} aces now, when for four aces you didn't? Forced choices contribute factor 1 --- spot them and skip them.}
...
%── D2 ──────────────────────────────────────────────────────────────────────────
\item Subsets of $\{1,\ldots,8\}$ with no two consecutive integers (empty set included).
...
%── D3 ──────────────────────────────────────────────────────────────────────────
\item Committees of 4 from 7 juniors and 5 seniors with strictly more juniors.

\ans{$\dbinom{7}{4}+\dbinom{7}{3}\dbinom{5}{1}=35+175=210$}
\method{Strictly more juniors in a committee of 4 means 4--0 or 3--1. Two cases, add: $35+35\cdot5$.}
\inv{Note the impossibility of a tie playing no role here --- but for committees of 4 with a 2--2 split allowed, ``more juniors'' vs ``more seniors'' vs ``tie'' partitions $\binom{12}{4}=495$. Compute all three and check they sum correctly. Symmetry does NOT make the first two equal --- why?}
